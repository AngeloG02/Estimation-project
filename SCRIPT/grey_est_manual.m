...
function [theta_est,std_theta] = grey_est_manual (sigma_q,sigma_ax)
...
% ---> DEFINIAMO NFFT PER AVERE PIÙ PUNTI <---
NFFT = 16384; 
...
% ---> INSERIAMO NFFT AL POSTO DELLE PARENTESI VUOTE [] <---
% Input Autospectrum (Suu)
[Suu, f] = pwelch(u_no_mean, win, noverlap, NFFT, fs);
% Cross-spectrum between input (u) and pitch rate (q)
[Suq, ~] = cpsd(u_no_mean, q_no_mean, win, noverlap, NFFT, fs);
% Cross-spectrum between input (u) and longitudinal acceleration (ax)
[Suax, ~] = cpsd(u_no_mean, ax_no_mean, win, noverlap, NFFT, fs);
...
% ---> INSERIAMO NFFT ANCHE QUI <---
[Sqq, ~] = pwelch(q_no_mean, win, noverlap, NFFT, fs);
[Saxax, ~] = pwelch(ax_no_mean, win, noverlap, NFFT, fs);
...
%% --- FILTRAGGIO DATI PER L'OTTIMIZZAZIONE ---
% Ora freq_vect ha migliaia di punti grazie a NFFT = 16384. 
% Prendiamo solo quelli nel range desiderato.
idx_opt = (freq_vect >= f_min) & (freq_vect <= f_max);

freq_vect_filtered = freq_vect(idx_opt);
G_m_filtered = G_m(idx_opt, :);

fprintf('\n--- FILTERING DATA FOR OPTIMIZATION ---\n');
fprintf('Selected frequency range: %.2f Hz to %.2f Hz\n', freq_vect_filtered(1), freq_vect_filtered(end));
...
std_theta =1; % così non da errore 





% Se devi calcolare la standard deviation dalla matrice Hessiana:
% std_theta = sqrt(diag(inv(hessian_final)));
% 
% % =============================================
% %  STEP 3: Crea iddata
% %  =============================================
