function [theta_est,std_theta] = grey_est_modificato (sigma_q,sigma_ax)
...
semilogx(f, 20*log10(abs(Suq)));
grid on;
title(sprintf('CSPD q (σ_q=%.2f°/s, σ_ax=%.4f m/s²)', sigma_q, sigma_ax));
ylabel('Magnitude [dB]');
...
semilogx(f, 20*log10(abs(Suax)));
grid on;
title(sprintf('CSPD ax (σ_q=%.2f°/s, σ_ax=%.4f m/s²)', sigma_q, sigma_ax));
xline(f_min, 'r--', 'LineWidth', 2.5, 'Label', sprintf('f_{min}=%.2f Hz', f_min));
xline(f_max, 'g--', 'LineWidth', 2.5, 'Label', sprintf('f_{max}=%.2f Hz', f_max));
...
semilogx(f, gamma_sq_q);
hold on;
plot([f(1), f(end)], [0.7, 0.7], 'r--', 'LineWidth', 1.5);
grid on;
title(sprintf('Coherence: Input (u) to Pitch Rate (q) (σ_q=%.2f°/s, σ_ax=%.4f m/s²)', sigma_q, sigma_ax));
ylim([0 1.05]);
ylabel('Coherence \gamma^2');
legend('Coherence', 'Trust Threshold');
xline(f_min, 'r--', 'LineWidth', 2.5, 'Label', sprintf('f_{min}=%.2f Hz', f_min));
xline(f_max, 'g--', 'LineWidth', 2.5, 'Label', sprintf('f_{max}=%.2f Hz', f_max));
...
plot([f(1), f(end)], [0.7, 0.7], 'r--', 'LineWidth', 1.5);
grid on;
title(sprintf('Coherence: Input (u) to Longitudinal Accel. (ax) (σ_q=%.2f°/s, σ_ax=%.4f m/s²)', sigma_q, sigma_ax));
ylim([0 1.05]);
xlabel('Frequency [Hz]');
ylabel('Coherence \gamma^2');
xline(f_min, 'r--', 'LineWidth', 2.5, 'Label', sprintf('f_{min}=%.2f Hz', f_min));
...
%% =============================================
%  STEP 7: SELEZIONA INTERVALLO DI FREQUENZE
% =============================================
...
%% =============================================
%  STEP 9: ESEGUI GREYEST CON DATI FREQUENZIALI
% =============================================
...
z_1sigma = 1.000;   %#ok<NASGU>
z_2sigma = 1.960;   % 95% CI
z_3sigma = 2.576;   %#ok<NASGU>

if all(~isnan(std_theta))
    ...
    fprintf('\n95%% Confidence Intervals not available: std_theta not computed.\n');
end
...
figure('Position', [100, 100, 1400, 900], ...
       'Name', sprintf('Compare: Data vs Model (σ_q=%.2f°/s, σ_ax=%.4f m/s²)', sigma_q, sigma_ax));
...
for i = 1:length(h.Children)
    ...
    if isa(axh, 'matlab.graphics.axis.Axes')
        axh.FontSize = 10;
        axh.YLabel.FontSize = 10;
        axh.XLabel.FontSize = 10;
        axh.Title.FontSize = 11;
    end
end

% Dati del Bode del modello identificato
[mag, phase, wout] = bode(sys);

% Estrazione dati dal bode (reshape da 3D a 2D)
mag_q    = squeeze(mag(1, 1, :));      % Pitch rate magnitude
mag_ax_m = squeeze(mag(2, 1, :));      % Acceleration magnitude
phase_q  = squeeze(phase(1, 1, :));    % Pitch rate phase
phase_ax = squeeze(phase(2, 1, :));    % Acceleration phase
freq_bode_Hz = wout / (2*pi);          % Converti da rad/s a Hz

figure('Position', [100, 100, 1200, 800], ...
       'Name', sprintf('FRF Welch - Bode (σ_q=%.2f°/s, σ_ax=%.4f m/s²)', sigma_q, sigma_ax));

% Magnitude plots
subplot(2, 2, 1);
semilogx(freq_vect, 20*log10(abs(H_q_real_sys)), 'b-', 'LineWidth', 2);
hold on
semilogx(freq_bode_Hz, 20*log10(mag_q), 'r-', 'LineWidth', 2);
grid on;
...
hold on
semilogx(freq_bode_Hz, 20*log10(mag_ax_m), 'r-', 'LineWidth', 2);
grid on;
...
hold on
semilogx(freq_bode_Hz, phase_q, 'r-', 'LineWidth', 2);
grid on;
xlabel('Frequency [Hz]');
ylabel('Phase [°]');
...
hold on
semilogx(freq_bode_Hz, phase_ax, 'r-', 'LineWidth', 2);
grid on;
xlabel('Frequency [Hz]');
ylabel('Phase [°]');
